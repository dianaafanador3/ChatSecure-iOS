...
- (void)connection:(__unused NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    {
        [self finish];
        if (self.completionHandler) self.completionHandler([req valueForHTTPHeaderField:@"TransferId"], self.responseReceived, self.accumulatedData, error);
    }
}
...
- (void)connection:(__unused NSURLConnection *)connection didSendBodyData:(__unused NSInteger)bytesWritten totalBytesWritten:(NSInteger)totalBytesWritten totalBytesExpectedToWrite:(NSInteger)totalBytesExpectedToWrite
{
    ...
    if (self.uploadProgressHandler)
    {
        ...
        self.uploadProgressHandler([req valueForHTTPHeaderField:@"TransferId"], progress, totalBytesWritten, totalBytesExpectedToWrite);
    }
}
...
- (void)connection:(__unused NSURLConnection *)connection didReceiveData:(NSData *)data
{
    ...
    if (self.downloadProgressHandler)
    {
        ...
        self.downloadProgressHandler([req valueForHTTPHeaderField:@"TransferId"], (float)bytesTransferred / (float)totalBytes, bytesTransferred, totalBytes);
    }
}
...
- (void)connectionDidFinishLoading:(__unused NSURLConnection *)_connection
{
    ...
    NSError *error = nil;
    NSURLRequest *req = _connection.originalRequest;
